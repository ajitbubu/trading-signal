...
`evaluate_position` returns the firing signals for a held position.
`evaluate_candidate` evaluates a screener-qualified ticker for entry.
Each signal carries the rule that fired so the UI can surface it.
"""
    ...
    *,
    ...
    if avg_cost > 0:
        ...
        if change <= stop_loss_pct:
            out.append(Signal(ticker, "exit", "stop_loss", f"{change:.1%} ≤ {stop_loss_pct:.0%}", True))
        if change >= profit_target_pct:
            out.append(
                Signal(ticker, "exit", "profit_target", f"{change:.1%} ≥ {profit_target_pct:.0%}", True)
            )
